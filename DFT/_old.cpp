...
void dft(std::vector<std::complex<double>>::iterator rb, std::vector<std::complex<double>>::iterator re, 
	...
	unsigned k{};
	unsigned N = std::distance(rb, re);
	double C = 2 * _Pi * N;
	unsigned n{};
	std::complex<double> t_sum(0.,0.);	// complex with simple range type

	for (auto i(outb); i != oute; i++) {	// iterate output range
		n ++;
		for (auto j(rb); j != re; j++) {	// iterate input range
			complex<double> temp( cos(C * k * n), -sin(C * k * n) );
			t_sum += *j * temp;
			k++;
		}
		*i = t_sum;
...
int main() {
	...
	std::fill(vec0.begin(), vec0.end(), complex<double>{ -17.64, 23.52 });
	std::generate(vec1.begin(), vec1.end(), Random::Cmplx1);
